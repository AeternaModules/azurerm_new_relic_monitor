...
output "new_relic_monitors_identity" {
  ...
  value       = { for k, v in azurerm_new_relic_monitor.new_relic_monitors : k => one(v.identity) if v.identity != null && length(v.identity) > 0 }
}
...
output "new_relic_monitors_plan" {
  ...
  value       = { for k, v in azurerm_new_relic_monitor.new_relic_monitors : k => one(v.plan) if v.plan != null && length(v.plan) > 0 }
}
...
output "new_relic_monitors_user" {
  ...
  value       = { for k, v in azurerm_new_relic_monitor.new_relic_monitors : k => one(v.user) if v.user != null && length(v.user) > 0 }
}
